...
> **用途**: Phase -1〜2 における AI Agent との協働 Coding Workflow の参考資料。Phase -1 の「AI 協働ルール」コンポーネントで参照する。
> **注意**: このドキュメントは `agentic-workflow-guide.md` の拡張版である。`../../docs/Contract.md` の方針（vibe-local がコミット・PR作成を行い、人間は Merge のみ判定する）に従って更新されている。
...
## Phase 0: AI Context File Setup（このガイド固有の概念）

> **注意**: この「Phase 0」は `docs/Contract.md` の「Phase 0（Zero Trust 基盤の最小実装）」とは別の概念である。このガイドにおける Phase 0 は、AI コンテキストファイル（`AGENTS.md` 等）のセットアップを指す。`docs/Contract.md` の実装フェーズとの対応は `docs/Contract.md` → `## 実装ロードマップ` → `### Phase -1 実装仕様` → コンポーネント8「AI 協働ルール」を参照。
...
6. Commit and create PR (vibe-local performs this autonomously)

**The agent (vibe-local):**
...
- **Commits cleanup changes and creates PR** after all gates pass (vibe-local's responsibility)
...
# vibe-local commits and creates PR; paid AI reviews, human makes final Merge decision

/wf-03-implement issue-47-user-email-validation task-2
# vibe-local commits and creates PR; paid AI reviews, human makes final Merge decision

/wf-04-cleanup issue-47-user-email-validation
# vibe-local commits cleanup and creates PR; paid AI reviews, human makes final Merge decision
```
...
| **Merging PR without human review** | Bypasses human review gate | vibe-local commits and creates PR; human reviews and merges |
...
10. **Agent proposes, human approves** -- Scope changes, dependencies, and architectural decisions require explicit approval. Commits and PR creation are performed autonomously by vibe-local; human approval is required for Merge only.
